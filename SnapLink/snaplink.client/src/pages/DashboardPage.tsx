...
function UrlCard({ url, onDelete, onCopy }: {
  ...
  onDelete: (id: string) => void; // FIX: id is a UUID string, not a number
...
export default function DashboardPage() {
  ...
  async function handleDelete(id: string) { // FIX: string UUID, not number
    await deleteUrl(id);
    setUrls(prev => prev.filter(u => u.id !== id));
    // FIX: Both sides are now string — previously compared string to number
    // so result banner never cleared after deleting the just-created link.
